...
[center,point2cluster,Cell] = meanshift(X',bandwidth);% MeanShift
for i = 1:length(Cell)% Replace Image Colors
X(Cell{i},:) = repmat(center(:,i)',size(Cell{i},2),1); 
end
...
%% MeanShift
function [clustCent,data2cluster,cluster2dataCell] = meanshift(datas,bandWidth)
...
%**** Initialize stuff ***
[numDim,numPts] = size(datas);
numClust        = 0;
bandSq          = bandWidth^2;
initPtInds      = 1:numPts;
maxPos          = max(datas,[],2);                      %biggest size in each dimension
minPos          = min(datas,[],2);                      %smallest size in each dimension
boundBox        = maxPos-minPos;                        %bounding box size
sizeSpace       = norm(boundBox);                       %indicator of size of data space
stopThresh      = 1e-3*bandWidth;                       %converged thresh hold: 0.0001
...
while numInitPts
    ...
    myMean          = datas(:,stInd);                       % intilize mean to this points location
    myMembers       = [];                                   % points that will get added to this cluster                          
    thisClusterVotes    = zeros(1,numPts,'uint16');         %used to resolve conflicts on cluster membership

    while 1     %loop untill convergence
        
        sqDistToAll = sum((repmat(myMean,1,numPts) - datas).^2);    %dist squared from mean to all points still active
        inInds      = find(sqDistToAll < bandSq);               %points within bandWidth
        thisClusterVotes(inInds) = thisClusterVotes(inInds)+1;  %add a vote for all the in points belonging to this cluster
        
        
        myOldMean   = myMean;                                   %save the old mean
        myMean      = mean(datas(:,inInds),2);                %compute the new mean
        ...
        if plotFlag
            ...
            if numDim == 2
                ...
                plot(datas(1,myMembers),datas(2,myMembers),'ys')
