...
tic
%% input
v = VideoReader('0621demo/focus.mp4');% Read the input video
frame = v.NumberOfFrames;
recf = 1;
for frame_n = [1:fix(frame/50):frame*99/100]
    I = read(v,frame_n);
    [height,width,dim] = size(I);
    while height>800 | width>800
        [height,width,dim] = size(I);
        I = imresize(I,[height/2 width/2]);
    end
    [height,width,dim] = size(I);
    I2 = I;
%% face detection
    ...
    record(recf,:) = [frame_n, facenum, skinpercent, quality];
    recf = recf + 1;
    %figure,imshow(I);  title(['Pornographic quality : ',num2str(quality),'%']); 
end
record = real(record);
%% iteration method
[val recd] = size(record);
for k = 2:val
    record(k,4) = record(k,4)*(1+(record(k-1,4)/100));
end
result=0;
for m = 1:val
    result = result + record(m,4);
end
result = result/val;
[max, max_index] = max(record(:,3));
result_img = read(v,record(max_index,1));
figure,imshow(result_img);  title(['Pornographic quality : ',num2str(result),'%']); 
toc